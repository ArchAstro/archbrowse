...
const root=await mkdtemp(join(tmpdir(),'react-kitty-persistent-e2e-'));
const env={...process.env,REACT_KITTY_SESSIONS_DIR:join(root,'sessions')};
...
try {
  ...
  const beforePopup=terminal.frames.at(-1)!;
  ...
  await waitFor(()=>!!terminal.frames.at(-1) && !samePixels(terminal.frames.at(-1)!,beforePopup),'popup frame');
  ...
  const activeFrame=terminal.frames.at(-1)!;

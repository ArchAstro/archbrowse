...
import { status } from '../../skills/starpane/scripts/bootstrap.mjs';
import { savePreferences, planLaunch } from '../../skills/starpane/scripts/preferences.mjs';
const exec=promisify(execFile),root=await mkdtemp(join(tmpdir(),'starpane-skill-e2e-'));
...
const env:NodeJS.ProcessEnv={...process.env,HERDR_ENV:'',HERDR_SESSION:'',HERDR_PANE_ID:'',HERDR_SOCKET_PATH:'',HERDR_CONFIG_PATH:config,STARPANE_SESSIONS_DIR:join(root,'sessions'),STARPANE_PREFERENCES_FILE:join(root,'preferences.json'),TERM:'xterm-ghostty',TERM_PROGRAM:'ghostty'};
for(const key of ['SSH_CONNECTION','SSH_TTY','TMUX','STY','HERDR_REMOTE_KEYBINDINGS'])delete env[key];
const host=new KittyHost(name,env);let socket='',report='# Starpane skill workflow\n\n';
const command=installed.command!;
async function starpane(args:string[]){return JSON.parse((await exec(command[0],[...command.slice(1),...args,'--json'],{env})).stdout);}
...
try {
  ...
  for(const placement of ['split','tab']){
    ...
    if(placement==='split'){
      ...
      const result=JSON.parse((await exec('herdr',['tab','create','--workspace',caller.workspace_id,'--cwd',plan.workspace,'--label','Starpane','--no-focus'],{env:context})).stdout);pane=result.result.root_pane.pane_id;tab=result.result.tab.tab_id;
    }
    ...
    await waitFor(async()=>{try{return (await starpane(['--session',plan.session,'attach'])).ok;}catch{return false;}},'installed Starpane ready in preferred location',20000);
    const tree=await starpane(['--session',plan.session,'snapshot','-i']);assert.ok(tree.result.refs.some((ref:{name:string})=>ref.name==='Count: 0'));
    ...
    const same=await planLaunch(process.cwd(),{},context);assert.equal(same.session,plan.session);assert.equal((await starpane(['--session',same.session,'attach'])).ok,true);
    ...
    await waitFor(async()=>{try{await starpane(['--session',plan.session,'attach']);return false;}catch{return true;}},'viewer closed');

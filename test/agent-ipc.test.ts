...
test('agent socket isolates names, preserves errors and disappears on shutdown',async()=> {
  const root=await mkdtemp(join(tmpdir(),'rk-agent-ipc-')),previous=process.env.REACT_KITTY_SESSIONS_DIR;
  process.env.REACT_KITTY_SESSIONS_DIR=join(root,'deliberately-long-profile-path-to-exercise-the-short-socket-fallback');
  ...
  }finally{await server?.close();if(previous===undefined)delete process.env.REACT_KITTY_SESSIONS_DIR;else process.env.REACT_KITTY_SESSIONS_DIR=previous;await rm(root,{recursive:true,force:true});}
});

...
test('agent socket isolates names, preserves errors and disappears on shutdown',async()=> {
  const root=await mkdtemp(join(tmpdir(),'archbrowse-agent-ipc-')),previous=process.env.ARCHBROWSE_SESSIONS_DIR;
  process.env.ARCHBROWSE_SESSIONS_DIR=join(root,'deliberately-long-profile-path-to-exercise-the-short-socket-fallback');
  ...
  }finally{await server?.close();if(previous===undefined)delete process.env.ARCHBROWSE_SESSIONS_DIR;else process.env.ARCHBROWSE_SESSIONS_DIR=previous;await rm(root,{recursive:true,force:true});}
});

...
test('named session lock, atomic metadata, private permissions and active deletion guard',async()=> {
  const root=await mkdtemp(join(tmpdir(),'react-kitty-session-unit-'));
  const original=process.env.REACT_KITTY_SESSIONS_DIR; process.env.REACT_KITTY_SESSIONS_DIR=root;
  ...
  try {
    ...
    if(original===undefined) delete process.env.REACT_KITTY_SESSIONS_DIR; else process.env.REACT_KITTY_SESSIONS_DIR=original;
